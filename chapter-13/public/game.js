(() => {
  ...
  const PROGRESS_STORAGE_KEY = "revelation-memory-game:chapter-13:progress";
  const COLORS = [
    "rgb(3, 126, 219)",
    "rgb(156, 113, 219)",
    "rgb(240, 144, 187)",
  ...
  const bgm = new Audio("/assets/audio/Revelation13BGM.mp3");
  bgm.loop = true;
  bgm.preload = "auto";
  bgm.volume = 0.22;
  ...
  function resetSentence() {
    ...
    state.displayLines = state.introVisible && state.verseIndex === 0
      ? [{ parts: [{ text: "요한계시록 13장", intro: true }] }]
      : [];
