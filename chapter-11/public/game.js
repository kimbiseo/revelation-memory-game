(() => {
  ...
  const PROGRESS_STORAGE_KEY = "revelation-memory-game:chapter-11:progress";
  const COLORS = [
    "rgb(3, 126, 219)",
    "rgb(156, 113, 219)",
    "rgb(240, 144, 187)",
  ...
  const verseChunks = [
    ...
    ["그 시에", "큰 지진이", "나서", "성 1/10이", "무너지고", "지진에", "죽은 사람이", "칠천이라", "그 남은 자들이", "두려워하여", "영광을", "하늘의 하나님께", "돌리더라"],
  ...
  const bgm = new Audio("/assets/audio/Revelation11BGM.mp3");
  bgm.loop = true;
  bgm.preload = "auto";
  bgm.volume = 0.22;
  ...
  function resetSentence() {
    ...
    state.displayLines = state.introVisible && state.verseIndex === 0
      ? [{ parts: [{ text: "요한계시록 11장", intro: true }] }]
      : [];
