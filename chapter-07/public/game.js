(() => {
  ...
  const PROGRESS_STORAGE_KEY = "revelation-memory-game:chapter-07:progress";
  const COLORS = [
    "rgb(3, 126, 219)",
    "rgb(156, 113, 219)",
    "rgb(240, 144, 187)",
  ...
  const verseChunks = [
    ...
    ["내가", "인맞은 자의", "수를 들으니", "이스라엘 자손의", "각 지파 중에서", "인맞은 자들이", "144000이니"],
  ...
  const bgm = new Audio("/assets/audio/Revelation7BGM.mp3");
  bgm.loop = true;
  bgm.preload = "auto";
  bgm.volume = 0.22;
  ...
  function resetSentence() {
    ...
    state.displayLines = state.introVisible && state.verseIndex === 0
      ? [{ parts: [{ text: "요한계시록 7장", intro: true }] }]
      : [];
