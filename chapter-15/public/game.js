(() => {
  ...
  const PROGRESS_STORAGE_KEY = "revelation-memory-game:chapter-15:progress";
  const COLORS = [
    "rgb(3, 126, 219)",
    "rgb(156, 113, 219)",
    "rgb(240, 144, 187)",
  ...
  const verseChunks = [
    ...
    ["네 생물 중에", "하나가", "세세에 계신", "하나님의 진노를", "가득히 담은", "금대접 7을", "그 7 천사에게", "주니"],
  ...
  const bgm = new Audio("/assets/audio/Revelation15BGM.mp3");
  bgm.loop = true;
  bgm.preload = "auto";
  bgm.volume = 0.22;
  ...
  function resetSentence() {
    ...
    state.displayLines = state.introVisible && state.verseIndex === 0
      ? [{ parts: [{ text: "요한계시록 15장", intro: true }] }]
      : [];
