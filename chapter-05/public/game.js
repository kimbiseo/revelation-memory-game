(() => {
  ...
  const PROGRESS_STORAGE_KEY = "revelation-memory-game:chapter-05:progress";
  const COLORS = [
    "rgb(3, 126, 219)",
    "rgb(156, 113, 219)",
    "rgb(240, 144, 187)",
  ...
  const verseChunks = [
    ...
    ["책을 취하시매", "네 생물과", "24 장로들이", "어린 양 앞에", "엎드려", "각각", "거문고와", "향이 가득한", "금 대접을", "가졌으니", "이 향은", "성도의", "기도들이라"],
  ...
  const bgm = new Audio("/assets/audio/Revelation5BGM.mp3");
  bgm.loop = true;
  bgm.preload = "auto";
  bgm.volume = 0.22;
  ...
  function resetSentence() {
    ...
    state.displayLines = state.introVisible && state.verseIndex === 0
      ? [{ parts: [{ text: "요한계시록 5장", intro: true }] }]
      : [];
