(() => {
  ...
  const PROGRESS_STORAGE_KEY = "revelation-memory-game:chapter-09:progress";
  const COLORS = [
    "rgb(3, 126, 219)",
    "rgb(156, 113, 219)",
    "rgb(240, 144, 187)",
  ...
  const verseChunks = [
    ...
    ["이 세 재앙", "곧", "저희 입에서", "나오는", "불과", "연기와", "유황을 인하여", "사람 1/3이", "죽임을", "당하니라"],
  ...
  const bgm = new Audio("/assets/audio/Revelation9BGM.mp3");
  bgm.loop = true;
  bgm.preload = "auto";
  bgm.volume = 0.22;
  ...
  function resetSentence() {
    ...
    state.displayLines = state.introVisible && state.verseIndex === 0
      ? [{ parts: [{ text: "요한계시록 9장", intro: true }] }]
      : [];
